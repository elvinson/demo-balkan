...
OrgChart.prototype.ripple = function (id, clientX, clientY) {
    ...
    rRect.setAttribute("width", t.ripple.rect ? t.ripple.rect.width : node.w);
    rRect.setAttribute("height", t.ripple.rect ? t.ripple.rect.height : node.h);
...
OrgChart.prototype.updateNode = function (node, callback, fireEvent) {
    ...
    this._draw(false, OrgChart.action.update, { id: pid }, function () {
        ...
        OrgChart.events.publish('updated', [this, oldNode, node]);
    });
};
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            item.addEventListener("click", function (e) {                
                ...
                if (onClick === undefined) {
                    if (item === "add") {
                        var data = { id: OrgChart.randomId(), pid: firstNodeId };
...
OrgChart.prototype._lonelyButtonHandler = function () {
    var node = new OrgChart.node(OrgChart.randomId(), null, [], this.config.template);
...
OrgChart.local._walk = function (roots, layoutConfigs) {
    ...
    //START do not change the tworoot iterations bug
    for (var i = 0; i < roots.length; i++) {
    ...
    //END do not change the tworoot iterations bug
...
OrgChart.server._initNode = function (node, nodes, layoutConfigName, level, action, layoutConfigs, config, expandedNodeIds, actionParams, partnerIds, state) {    
    ...
    if (node.tags.indexOf("assistant") != -1) {
        node.isAssistant = true;
    }
...
OrgChart.server._createNodes = function (config, layoutConfigs, action, actionParams, oldNodes, state) {
    ...
    for (var layoutConfigName in layoutConfigs) {
        ...
        if (layoutConfig.layout == OrgChart.mixed) {
            hasMixedLayout = true;
            break;
        }
    }
    if (hasMixedLayout) {
        for (var pid in lastChildrenPidIds) {
            ...
            var layoutConfig = layoutConfigs[configName];
            if (layoutConfig.layout == OrgChart.mixed) {
                var cnodeIds = lastChildrenPidIds[pid];
    ...
    for (var layoutConfigName in layoutConfigs) {
        var layoutConfig = layoutConfigs[layoutConfigName];
        if (layoutConfig.layout == OrgChart.tree || layoutConfig.layout == OrgChart.treeRightOffset || layoutConfig.layout == OrgChart.treeLeftOffset) {
            hasTreeLayout = true;
            break;
        }
    }
    ...
    if (hasTreeLayout) {
        for (var pid in lastChildrenPidIds) {
            var pnode = nodes[lastChildrenPidIds[pid][0]].parent; //parent node
            var configName = pnode.lcn ? pnode.lcn : "base";
            var layoutConfig = layoutConfigs[configName];
            ...
            for (var i = lastChildrenPidIds[pid].length - 1; i >= 0; i--) {

                var cnode = nodes[lastChildrenPidIds[pid][i]];
                ...
                if (layoutConfig.layout == OrgChart.treeRightOffset) {
                    tnodeIds.splice(0, 0, cnode.id);
                }
                if (layoutConfig.layout == OrgChart.treeLeftOffset || layoutConfig.layout == OrgChart.treeRightOffset) {
...
OrgChart.server._getResponse = function (width, height, oldVisibleNodeIds, config, maxX, maxY, minX, minY, viewBox, roots, action, actionParams, nodes, oldNodes, vbIsInitializedFromState) {
    ...
    if (viewBox == null && !vbIsInitializedFromState && config.align == OrgChart.CENTER) { //initial load viewBox  is null
...
OrgChart.accentFold = function(inStr) {
    ...
    return inStr.replace(
      /([Р“В Р“РЋР“СћР“Р€Р“В¤Р“Тђ])|([Р“В§])|([Р“РЃР“В©Р“Р„Р“В«])|([Р“В¬Р“В­Р“В®Р“Р‡])|([Р“В±])|([Р“Р†Р“С–Р“Т‘Р“ВµР“В¶Р“С‘])|([Р“Сџ])|([Р“в„–Р“С”Р“В»Р“С])|([Р“С—])|([Р“В¦])/g, 
